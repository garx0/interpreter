...
class Parser {
	...
	int curVal = 0;
	stack<LexT> stType;
	// используется для проверки соответствий типов.
	// также его использует процедура ntConst, занося одно значение,
	//   поэтому после каждого ее использования надо извлекать это
	//   значение, чтобы проверка соответствий типов работала корректно
	stack<int> stVal;
	// используется только процедурой ntConst
	vector<RpnOp*> rpn;
	int cycles = 0;
